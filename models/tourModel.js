...
const tourSchema = new mongoose.Schema(
  {
    ...
    priceDiscount: {
      type: Number,
      validate: {
        // this only points to current doc on NEW document creation, not updating it
        validator: function(val) {
          return val < this.price;
        },
        // It works because of MONGO itself (value property)
    ...
    startLocation: {
      // GeoJSON -> MongoDB uses that special data format for Geospatial Data (lng, lat)
      type: {
        type: String,
        default: 'Point',
        enum: ['Point']
